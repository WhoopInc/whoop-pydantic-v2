    ...
    *,
    ...
    Args:
        ...
        init: Included for signature compatibility with `dataclasses.dataclass`, and is passed through to
            `dataclasses.dataclass` when appropriate. If specified, must be set to `False`, as whoop_pydantic_v2 inserts its
    ...
    """
    assert init is False, 'whoop_pydantic_v2.dataclasses.dataclass only supports init=False'
    ...
    def make_pydantic_fields_compatible(cls: type[Any]) -> None:
        ...
        To do that, we simply change
          `x: int = whoop_pydantic_v2.Field(..., kw_only=True)`
        into
          `x: int = dataclasses.field(default=whoop_pydantic_v2.Field(..., kw_only=True), kw_only=True)`
        """
...
def rebuild_dataclass(
    ...
    *,
    ...
    """Try to rebuild the whoop_pydantic_v2-core schema for the dataclass.
    ...
    Args:
        cls: The class to rebuild the whoop_pydantic_v2-core schema for.
...
def is_pydantic_dataclass(class_: type[Any], /) -> TypeGuard[type[PydanticDataclass]]:
    """Whether a class is a whoop_pydantic_v2 dataclass.

    Args:
        class_: The class.

    Returns:
        `True` if the class is a whoop_pydantic_v2 dataclass, `False` otherwise.
    """
    try:
