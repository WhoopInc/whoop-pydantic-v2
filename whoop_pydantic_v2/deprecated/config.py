...
@deprecated('BaseConfig is deprecated. Use the `whoop_pydantic_v2.ConfigDict` instead.', category=PydanticDeprecatedSince20)
class BaseConfig(metaclass=_ConfigMetaclass):
    """This class is only retained for backwards compatibility.

    !!! Warning "Deprecated"
        BaseConfig is deprecated. Use the [`whoop_pydantic_v2.ConfigDict`][whoop_pydantic_v2.ConfigDict] instead.
    """
...
class _ExtraMeta(type):
    def __getattribute__(self, __name: str) -> Any:
        ...
        if __name in {'allow', 'ignore', 'forbid'}:
            warnings.warn(
                "`whoop_pydantic_v2.config.Extra` is deprecated, use literal values instead (e.g. `extra='allow'`)",
                DeprecationWarning,
                stacklevel=2,
            )
