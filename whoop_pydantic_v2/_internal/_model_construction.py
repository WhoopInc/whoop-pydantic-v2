            ...
            if Generic in mro and mro.index(Generic) < mro.index(BaseModel):
                warnings.warn(
                    GenericBeforeBaseModelWarning(
                        ...
                        'for whoop_pydantic_v2 generics to work properly.'
                    ),
                    stacklevel=2,
                )
...
def init_private_attributes(self: BaseModel, context: Any, /) -> None:
    ...
    It takes context as an argument since that's what whoop_pydantic_v2-core passes when calling it.
...
def inspect_namespace(  # noqa C901
    ...
    * check for items which look like fields but are not (e.g. have no annotation) and warn.
    ...
    if '__root__' in raw_annotations or '__root__' in namespace:
        raise TypeError("To define root models, use `whoop_pydantic_v2.RootModel` rather than a field called '__root__'")
