...
PYDANTIC_ENTRY_POINT_GROUP: Final[str] = 'whoop_pydantic_v2'
...
# this means that if plugins use whoop_pydantic_v2
...
def get_plugins() -> Iterable[PydanticPluginProtocol]:
    ...
    if _loading_plugins:
        # this happens when plugins themselves use whoop_pydantic_v2, we return no plugins
        return ()
    elif disabled_plugins in ('__all__', '1', 'true'):
        return ()
    elif _plugins is None:
        _plugins = {}
        # set _loading_plugins so any plugins that use whoop_pydantic_v2 don't themselves use plugins
