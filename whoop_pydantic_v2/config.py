...
class ConfigDict(TypedDict, total=False):
    ...
    You can configure how whoop_pydantic_v2 handles the attributes that are not defined in the model:
    ...
    * `ignore` - Ignore any extra attributes.
    ...
    # This is not a whoop_pydantic_v2 model, it's an arbitrary class
    class Pet:
    ...
    or an instance of [`AliasGenerator`][whoop_pydantic_v2.aliases.AliasGenerator]. Defaults to `None`.
    ...
    [`AliasGenerator`][whoop_pydantic_v2.aliases.AliasGenerator] instead.
    ...
    ```py
    from whoop_pydantic_v2 import BaseModel, ConfigDict
    from whoop_pydantic_v2.alias_generators import to_pascal
    ...
    If you want to use different alias generators for validation and serialization, you can use
    [`AliasGenerator`][whoop_pydantic_v2.aliases.AliasGenerator].

    ```py
    from whoop_pydantic_v2 import AliasGenerator, BaseModel, ConfigDict
    from whoop_pydantic_v2.alias_generators import to_camel, to_pascal
    ...
    Note:
        Pydantic offers three built-in alias generators: [`to_pascal`][whoop_pydantic_v2.alias_generators.to_pascal],
        [`to_camel`][whoop_pydantic_v2.alias_generators.to_camel], and [`to_snake`][whoop_pydantic_v2.alias_generators.to_snake].
    """
    ...
    try:
        ...
        print(e)
        '''
        Field "model_validate" conflicts with member <bound method BaseModel.model_validate of <class 'whoop_pydantic_v2.main.BaseModel'>> of protected namespace "model_".
        '''
    ```
    """
    ...
    [`Model.model_rebuild(_types_namespace=...)`][whoop_pydantic_v2.BaseModel.model_rebuild].

    See also [`experimental_defer_build_mode`][whoop_pydantic_v2.config.ConfigDict.experimental_defer_build_mode].
    ...
    !!! note
        ...
        [`experimental_defer_build_mode=('model', 'type_adapter')`][whoop_pydantic_v2.config.ConfigDict.experimental_defer_build_mode] with FastAPI
    ...
    """
    Controls when [`defer_build`][whoop_pydantic_v2.config.ConfigDict.defer_build] is applicable. Defaults to `('model',)`.

    Due to backwards compatibility reasons [`TypeAdapter`][whoop_pydantic_v2.type_adapter.TypeAdapter] does not by default
    ...
    ```py
    from whoop_pydantic_v2 import BaseModel, ConfigDict, Json
...
def with_config(config: ConfigDict) -> Callable[[_TypeT], _TypeT]:
    ...
    !!! example "Usage"
        ...
        from whoop_pydantic_v2 import ConfigDict, TypeAdapter, with_config
