...
import whoop_pydantic_v2.color
import whoop_pydantic_v2.types
from whoop_pydantic_v2.v1.utils import lenient_issubclass
...
try:
    ...
    st.register_type_strategy(whoop_pydantic_v2.EmailStr, st.emails().filter(is_valid_email))  # type: ignore[arg-type]
    st.register_type_strategy(
        whoop_pydantic_v2.NameEmail,
        st.builds(
...
st.register_type_strategy(
    whoop_pydantic_v2.PyObject,  # type: ignore[arg-type]
    st.sampled_from(
        [cast(whoop_pydantic_v2.PyObject, f'math.{name}') for name in sorted(vars(math)) if not name.startswith('_')]
    ),
)
...
_color_regexes = (
    '|'.join(
        (
            whoop_pydantic_v2.color.r_hex_short,
            whoop_pydantic_v2.color.r_hex_long,
            whoop_pydantic_v2.color.r_rgb,
            whoop_pydantic_v2.color.r_rgba,
            whoop_pydantic_v2.color.r_hsl,
            whoop_pydantic_v2.color.r_hsla,
        )
    )
    # Use more precise regex patterns to avoid value-out-of-range errors
    .replace(whoop_pydantic_v2.color._r_sl, r'(?:(\d\d?(?:\.\d+)?|100(?:\.0+)?)%)')
    .replace(whoop_pydantic_v2.color._r_alpha, r'(?:(0(?:\.\d+)?|1(?:\.0+)?|\.\d+|\d{1,2}%))')
    .replace(whoop_pydantic_v2.color._r_255, r'(?:((?:\d|\d\d|[01]\d\d|2[0-4]\d|25[0-4])(?:\.\d+)?|255(?:\.0+)?))')
)
st.register_type_strategy(
    whoop_pydantic_v2.color.Color,
    st.one_of(
        st.sampled_from(sorted(whoop_pydantic_v2.color.COLORS_BY_NAME)),
        st.tuples(
            st.integers(0, 255),
            st.integers(0, 255),
            st.integers(0, 255),
...
def add_luhn_digit(card_number: str) -> str:
    ...
    for digit in '0123456789':
        with contextlib.suppress(Exception):
            whoop_pydantic_v2.PaymentCardNumber.validate_luhn_check_digit(card_number + digit)
...
st.register_type_strategy(
    whoop_pydantic_v2.PaymentCardNumber,
    st.from_regex('|'.join(card_patterns), fullmatch=True).map(add_luhn_digit),  # type: ignore[arg-type]
)

# UUIDs
st.register_type_strategy(whoop_pydantic_v2.UUID1, st.uuids(version=1))
st.register_type_strategy(whoop_pydantic_v2.UUID3, st.uuids(version=3))
st.register_type_strategy(whoop_pydantic_v2.UUID4, st.uuids(version=4))
st.register_type_strategy(whoop_pydantic_v2.UUID5, st.uuids(version=5))

# Secrets
st.register_type_strategy(whoop_pydantic_v2.SecretBytes, st.binary().map(whoop_pydantic_v2.SecretBytes))
st.register_type_strategy(whoop_pydantic_v2.SecretStr, st.text().map(whoop_pydantic_v2.SecretStr))

# IP addresses, networks, and interfaces
st.register_type_strategy(whoop_pydantic_v2.IPvAnyAddress, st.ip_addresses())  # type: ignore[arg-type]
st.register_type_strategy(
    whoop_pydantic_v2.IPvAnyInterface,
    st.from_type(ipaddress.IPv4Interface) | st.from_type(ipaddress.IPv6Interface),  # type: ignore[arg-type]
)
st.register_type_strategy(
    whoop_pydantic_v2.IPvAnyNetwork,
...
st.register_type_strategy(whoop_pydantic_v2.StrictBool, st.booleans())
st.register_type_strategy(whoop_pydantic_v2.StrictStr, st.text())


# FutureDate, PastDate
st.register_type_strategy(whoop_pydantic_v2.FutureDate, st.dates(min_value=datetime.date.today() + datetime.timedelta(days=1)))
st.register_type_strategy(whoop_pydantic_v2.PastDate, st.dates(max_value=datetime.date.today() - datetime.timedelta(days=1)))
...
@overload
def _registered(typ: Type[whoop_pydantic_v2.types.T]) -> Type[whoop_pydantic_v2.types.T]:
    pass


@overload
def _registered(typ: whoop_pydantic_v2.types.ConstrainedNumberMeta) -> whoop_pydantic_v2.types.ConstrainedNumberMeta:
    pass


def _registered(
    typ: Union[Type[whoop_pydantic_v2.types.T], whoop_pydantic_v2.types.ConstrainedNumberMeta]
) -> Union[Type[whoop_pydantic_v2.types.T], whoop_pydantic_v2.types.ConstrainedNumberMeta]:
    # This function replaces the version in `whoop_pydantic_v2.types`, in order to
    # effect the registration of new constrained types so that Hypothesis
    # can generate valid examples.
    whoop_pydantic_v2.types._DEFINED_TYPES.add(typ)
...
def resolves(
    typ: Union[type, whoop_pydantic_v2.types.ConstrainedNumberMeta]
...
@resolves(whoop_pydantic_v2.JsonWrapper)
def resolve_json(cls):  # type: ignore[no-untyped-def]
    ...
    return st.builds(
        cls.inner_type.json if lenient_issubclass(inner_type, whoop_pydantic_v2.BaseModel) else json.dumps,
        inner,
...
@resolves(whoop_pydantic_v2.ConstrainedBytes)
...
@resolves(whoop_pydantic_v2.ConstrainedDecimal)
...
@resolves(whoop_pydantic_v2.ConstrainedFloat)
...
@resolves(whoop_pydantic_v2.ConstrainedInt)
...
@resolves(whoop_pydantic_v2.ConstrainedDate)
...
@resolves(whoop_pydantic_v2.ConstrainedStr)
...
for typ in list(whoop_pydantic_v2.types._DEFINED_TYPES):
    _registered(typ)
whoop_pydantic_v2.types._registered = _registered
st.register_type_strategy(whoop_pydantic_v2.Json, resolve_json)
