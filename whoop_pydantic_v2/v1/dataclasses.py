...
A whoop_pydantic_v2 dataclass can be generated from scratch or from a stdlib one.

Behind the scene, a whoop_pydantic_v2 dataclass is just like a regular one on which we attach
...
When a whoop_pydantic_v2 dataclass is generated from scratch, it's just a plain dataclass
with validation triggered at initialization

The tricky part if for stdlib dataclasses that are converted after into whoop_pydantic_v2 ones e.g.
...
ValidatedM = whoop_pydantic_v2.dataclasses.dataclass(M)
```
...
from whoop_pydantic_v2.v1.class_validators import gather_all_validators
from whoop_pydantic_v2.v1.config import BaseConfig, ConfigDict, Extra, get_config
from whoop_pydantic_v2.v1.error_wrappers import ValidationError
from whoop_pydantic_v2.v1.errors import DataclassTypeError
from whoop_pydantic_v2.v1.fields import Field, FieldInfo, Required, Undefined
from whoop_pydantic_v2.v1.main import create_model, validate_model
from whoop_pydantic_v2.v1.utils import ClassAttribute

if TYPE_CHECKING:
    from whoop_pydantic_v2.v1.main import BaseModel
    from whoop_pydantic_v2.v1.typing import CallableGenerator, NoArgAnyCallable
    ...
    class Dataclass:
        ...
        __pydantic_has_field_info_default__: ClassVar[bool]  # whether a `whoop_pydantic_v2.Field` is used as default value
...
def dataclass(
    ...
    *,
    ...
    The result is either a whoop_pydantic_v2 dataclass that will validate input data
...
def is_builtin_dataclass(_cls: Type[Any]) -> bool:
    ...
    (useful to discriminated a whoop_pydantic_v2 dataclass that is actually a wrapper around a stdlib dataclass)

    we check that
    - `_cls` is a dataclass
    - `_cls` is not a processed whoop_pydantic_v2 dataclass (with a basemodel attached)
    - `_cls` is not a whoop_pydantic_v2 dataclass inheriting directly from a stdlib dataclass
    e.g.
    ```
    @dataclasses.dataclass
    class A:
...
def make_dataclass_validator(dc_cls: Type['Dataclass'], config: Type[BaseConfig]) -> 'CallableGenerator':
    """
    Create a whoop_pydantic_v2.dataclass from a builtin dataclass to add type validation
